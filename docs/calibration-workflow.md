# Calibration Workflow

The calibration workbench is the core tool. Section ranges, event ranges, and taps are independent tools that can be used in any order.

## Song Map Shape

Each song JSON keeps only the data the app needs:

- song identity
- genre: `SALSA` or `BACHATA`
- sections
- timeline events
- raw anchor taps
- reviewed count anchors
- status: `DRAFT` or `READY`

Sections and events share the same range model: `startTimeMs`, `endTimeMs`, `category`, and `tags`. Raw taps are manual anchor evidence. Reviewed anchors interpret those taps as genre-aware counts: Salsa uses `1` and `5`; Bachata uses `1`, tap step `4`, `5`, and tap step `8`. Categories and tags are global static JSON collections and can be reused anywhere.

## Ingestion

The local ingestion script accepts a YouTube id/link flow from the app and creates an empty calibration record. It does not infer BPM, sections, events, or taps.

## Workbench Behavior

- The timeline can be clicked or dragged to move the playhead.
- The playhead can be dragged all the way back to `0:00`.
- Section handles resize adjacent section ranges.
- Clicking a section selects it without moving the playhead.
- Events use the same start/end range model as sections.
- A local metronome calibration stores user/device input latency outside song JSON.
- Song tapping records one-key manual anchors, corrected by the saved local latency.
- Review labels anchors with genre-aware count options, nudges timing, deletes bad taps, and marks uncertainty.
- No library or automatic process generates trusted taps or downbeats.

## Publishing

Draft songs can remain partially calibrated. Publishing marks a song as `READY` after the schema accepts the current song map.
