...
def main():
    parser = argparse.ArgumentParser(description="Madmom RNN/DBN Beat Tracker")
    ...
    if not youtube_id:
        # Extract YouTube ID from output filename, e.g., 66HCBysrJS8_madmom.json -> 66HCBysrJS8
    ...
    print(f"[MADMOM] Loading madmom and processing audio: {audio_path} (ID: {youtube_id}, Title: {title})")
    start_time = time.time()
    
    # Import inside main to ensure libraries are loaded dynamically
    import madmom
    
    # Setup the RNN beat processor and DBN beat tracker
    act = madmom.features.beats.RNNBeatProcessor()(audio_path)
    proc = madmom.features.beats.DBNBeatTrackingProcessor(fps=100)
    beat_times = proc(act)
    
    print(f"[MADMOM] Processing completed in {time.time() - start_time:.2f}s.")
    print(f"[MADMOM] Found {len(beat_times)} beats.")
    ...
    tracker_display = "Madmom"
    stem_display = ""
    id_suffix = "madmom"
    if "_drums_" in output_path:
        stem_display = "Drums - "
        id_suffix = "drums-madmom"
    elif "_bass_" in output_path:
        stem_display = "Bass - "
        id_suffix = "bass-madmom"
    ...
    print(f"[SUCCESS] Exported madmom beatmap to {output_path}")
