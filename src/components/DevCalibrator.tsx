...
import {
	...
	TagCollectionSchema,
	createVocabularySongMapSchema,
} from '../types/schemas';
import {
	addDanceEvent,
	removeDanceEvent,
	type DanceEventDraft,
} from '../utils/danceEvents';
...
export default function DevCalibrator({
	...
	const [verificationGroupId, setVerificationGroupId] = useState<string | null>(
		null,
	);
	const [expandedGroups, setExpandedGroups] = useState<Record<string, boolean>>(
		{},
	);
	const [activeTapSectionId, setActiveTapSectionId] = useState<string | null>(
		null,
	);
	...
	const [focusedEventIndex, setFocusedEventIndex] = useState<number | null>(
		null,
	);
	...
	const [timelineView, setTimelineView] = useState<
		'sections' | 'events' | 'taps' | 'all'
	>('sections');
	const [eventTimelineScope, setEventTimelineScope] = useState<
		'song' | 'section'
	>('song');
	...
	const vocabularyModalTitle =
		vocabularyModal === 'category' ? 'New Category' : 'New Tag';
	...
	const anchorLabel = (count: number) =>
		reviewedAnchorOptions.find(option => option.count === count)?.label ||
		String(count);
	const tapById = useMemo(
		() => new Map(taps.map((tap: any) => [tap.id, tap])),
		[taps],
	);
	const passById = useMemo(
		() => new Map(tapCalibrationPasses.map((pass: any) => [pass.id, pass])),
	...
	const tapGroups = useMemo(() => {
		const anchors = [...activeReviewedAnchors].sort(
		...
		const gaps = anchors
			.slice(1)
			.map((anchor, index) => anchor.timeMs - anchors[index].timeMs);
		const localMedianGap =
			median(gaps.filter(gap => gap <= 6000)) || median(gaps) || 0;
		const splitThresholdMs =
			localMedianGap > 0 ? Math.min(3200, localMedianGap * 1.45) : 3200;
		const rawGroups: any[][] = [];
		...
		anchors.forEach((anchor, index) => {
			if (
				index > 0 &&
				anchor.timeMs - anchors[index - 1].timeMs > splitThresholdMs
			) {
		...
		return rawGroups.map((group, index) => {
			const groupGaps = group
				.slice(1)
				.map(
					(anchor, anchorIndex) => anchor.timeMs - group[anchorIndex].timeMs,
				);
			const previousGroupAnchor = rawGroups[index - 1]?.at(-1);
			const nextGroupAnchor = rawGroups[index + 1]?.[0];
			const gapBefore = previousGroupAnchor
				? group[0].timeMs - previousGroupAnchor.timeMs
				: 0;
			const gapAfter = nextGroupAnchor
				? nextGroupAnchor.timeMs - group.at(-1).timeMs
				: 0;
			...
			const cycleGaps = cycleAnchors
				.slice(1)
				.map(
					(anchor: any, anchorIndex: number) =>
						anchor.timeMs - cycleAnchors[anchorIndex].timeMs,
				);
			const invalidCycle = cycleAnchors
				.slice(1)
				.some(
					(anchor: any, anchorIndex: number) =>
						anchor.count !== expectedNextCount(cycleAnchors[anchorIndex].count),
				);
			const hasUncertain = group.some(
				anchor => anchor.confidence === 'uncertain',
			);
			const hasSuggested = group.some(
				anchor => !anchor.reviewed || anchor.confidence === 'suggested',
			);
			...
			const passIds = Array.from(
				new Set(groupTaps.map((tap: any) => tap.passId)),
			);
			let passDisagreement = false;
			if (passIds.length > 1) {
				const basePassId = passIds[0];
				const baseTaps = groupTaps.filter(
					(tap: any) => tap.passId === basePassId,
				);
				const repairTaps = groupTaps.filter(
					(tap: any) => tap.passId !== basePassId,
				);
				passDisagreement = repairTaps.some((tap: any) => {
					const closest = baseTaps.reduce(
						(best: number, baseTap: any) =>
							Math.min(
								best,
								Math.abs(baseTap.correctedTimeMs - tap.correctedTimeMs),
							),
						Infinity,
					);
					return closest > 150;
			...
			let confidence = 'high';
			if (
				cycleAnchors.length < 3 ||
				invalidCycle ||
				hasLargeInternalGap ||
				hasUncertain ||
				passDisagreement ||
				(index === 0 && group[0].timeMs < 20000) ||
				((gapBefore > splitThresholdMs || gapAfter > splitThresholdMs) &&
					cycleAnchors.length <= 5)
			) {
				confidence = 'low';
	...
	const verificationGroup = useMemo(() => {
		if (verificationGroupId) {
			const matchedGroup = tapGroups.find(
				(group: any) => group.id === verificationGroupId,
			);
	...
	const verificationAnchors = useMemo(() => {
		...
		return [...verificationGroup.anchors].sort(
	...
	const sectionStructureReady = useMemo(() => {
		...
		if (
			sections[0].startTimeMs > 250 ||
			sections.at(-1).endTimeMs < songEndMs - 250
		)
			return false;
	...
	const tapSectionCards = useMemo(() => {
			...
			const hiddenEvents = calibrationEvents.filter(
				(event: any) =>
					event.startTimeMs < section.endTimeMs &&
					event.endTimeMs > section.startTimeMs &&
					(event.tags || []).includes(HIDE_DOWNBEATS_TAG),
			);
			const markedNotNeeded = tapSectionDecisions.some(
				(decision: any) =>
					decision.sectionId === section.id && decision.status === 'not_needed',
			);
			const sectionHidden =
				(section.tags || []).includes(HIDE_DOWNBEATS_TAG) ||
				['intro', 'outro'].includes(section.category);
			let status = 'Needs Calibration';
			if (markedNotNeeded) {
				status = 'Marked Not Needed';
			} else if (
				sectionAnchors.some(
					(anchor: any) => anchor.reviewed && anchor.confidence === 'confirmed',
				)
			) {
				status = 'Calibrated';
	...
	const visibleTimeline = useMemo(() => {
		...
		return {
			startTimeMs: Math.max(
				0,
				Math.min(timelineZoom.startTimeMs, songEndMs - 1000),
			),
	...
	const visibleDurationMs = Math.max(
		1000,
		visibleTimeline.endTimeMs - visibleTimeline.startTimeMs,
	);
	const timelinePct = (timeMs: number) =>
		((timeMs - visibleTimeline.startTimeMs) / visibleDurationMs) * 100;
	const timelineWidthPct = (startTimeMs: number, endTimeMs: number) =>
		((endTimeMs - startTimeMs) / visibleDurationMs) * 100;
	const timelineRangeVisible = (startTimeMs: number, endTimeMs: number) =>
		endTimeMs >= visibleTimeline.startTimeMs &&
		startTimeMs <= visibleTimeline.endTimeMs;
	const clampVisibleTime = (timeMs: number) =>
		Math.max(
			visibleTimeline.startTimeMs,
			Math.min(visibleTimeline.endTimeMs, timeMs),
		);
	const timelineModeShowsSections =
		activeTab === 2
			? eventTimelineScope === 'section'
			: timelineView === 'sections' ||
				timelineView === 'events' ||
				timelineView === 'taps' ||
				(timelineView === 'all' &&
					(timelineLayers.sections || activeTab === 3));
	const timelineModeShowsEvents =
		activeTab === 2 ||
		timelineView === 'events' ||
		timelineView === 'taps' ||
		(timelineView === 'all' && timelineLayers.events);
	const timelineModeShowsRawTaps =
		timelineView === 'taps' ||
		(timelineView === 'all' && timelineLayers.rawTaps);
	const timelineModeShowsReviewed =
		timelineView === 'taps' ||
		(timelineView === 'all' && timelineLayers.reviewed);
	...
	useEffect(() => {
		try {
			...
			if (stored) {
				...
				if (
					Number.isFinite(parsed?.inputLatencyMs) &&
					Number.isFinite(parsed?.consistencyMs)
				) {
	...
	useEffect(() => {
		...
		if (activeTab === 3) {
			...
			const section =
				editorSections.find(sec => sec.id === activeTapSectionId) ||
				editorSections.find(
					sec =>
						liveDisplayTime * 1000 >= sec.startTimeMs &&
						liveDisplayTime * 1000 <= sec.endTimeMs,
				) ||
				editorSections[0];
			if (section) {
	...
	useEffect(() => {
		...
		const liveTimeMs = liveDisplayTime * 1000;
		if (
			liveTimeMs > section.endTimeMs - 80 ||
			liveTimeMs < section.startTimeMs - 250
		) {
	...
	useEffect(() => {
		...
		const liveTimeMs = liveDisplayTime * 1000;
		if (
			liveTimeMs >= visibleTimeline.startTimeMs &&
			liveTimeMs <= visibleTimeline.endTimeMs
		)
			return;
		...
		const startTimeMs = Math.max(
			0,
			Math.min(songEndMs - windowSize, liveTimeMs - windowSize * 0.35),
		);
	...
	useEffect(() => {
		...
		const nextPasses = Array.isArray(songData.tapCalibrationPasses)
			? songData.tapCalibrationPasses
			: [];
		setTapCalibrationPasses(nextPasses);
		setTaps(
			Array.isArray(songData.taps)
				? songData.taps.sort(
						(a: any, b: any) => a.correctedTimeMs - b.correctedTimeMs,
					)
				: [],
		);
		setReviewedAnchors(
			Array.isArray(songData.reviewedAnchors)
				? songData.reviewedAnchors.sort((a: any, b: any) => a.timeMs - b.timeMs)
				: [],
		);
	...
	const updateTapsState = (nextTaps: any[], triggerAutoSave = false) => {
		const sortedTaps = [...nextTaps].sort(
			(a, b) => a.correctedTimeMs - b.correctedTimeMs,
		);
		const updated = {
			...latestSongDataRef.current,
			taps: sortedTaps,
	...
	const updateTapCalibrationState = (
		nextPasses: any[],
		nextTaps: any[],
		nextAnchors: any[],
		triggerAutoSave = false,
	...
	const createCategory = (label: string) => {
		...
		const nextCategories = [...categories, {id, label: label.trim()}].sort(
			(a, b) => a.label.localeCompare(b.label),
		);
		setCategories(nextCategories);
		saveVocabulary(
			'/api/categories',
			{schemaVersion: '1.0', categories: nextCategories},
			'Category saved.',
		);
		return true;
	};
	...
	const createTag = (label: string) => {
		...
		const nextTags = [...tags, {id, label: label.trim()}].sort((a, b) =>
			a.label.localeCompare(b.label),
		);
		setTags(nextTags);
		saveVocabulary(
			'/api/tags',
			{schemaVersion: '1.0', tags: nextTags},
			'Tag saved.',
		);
		return true;
	};
	...
	const stopMetronomeCalibration = () => {
		...
		const average =
			metronomeSamples.reduce((sum, value) => sum + value, 0) /
			metronomeSamples.length;
		const variance =
			metronomeSamples.reduce(
	...
	const createReplacementSectionTapPass = (section: any) => {
			...
			taps
				.filter(
					(tap: any) =>
						...
						(tap.correctedTimeMs >= section.startTimeMs &&
							tap.correctedTimeMs <= section.endTimeMs),
				)
				.map((tap: any) => tap.id),
		);
		...
		const nextPasses = [
			...tapCalibrationPasses.filter(
				(existingPass: any) =>
					!oldSectionPassIds.has(existingPass.id) &&
					remainingPassIds.has(existingPass.id),
			),
			pass,
		];
	...
	const suggestedCountForTap = (
		timeMs: number,
		passId: string,
		anchorSource = reviewedAnchors,
		tapSource = taps,
	) => {
		const existingAnchors = [...anchorSource]
			.filter(
				anchor =>
					tapSource.find((tap: any) => tap.id === anchor.tapId)?.passId ===
					passId,
			)
			.sort((a, b) => a.timeMs - b.timeMs);
		const previous = existingAnchors
			.filter(anchor => anchor.timeMs < timeMs)
			.at(-1);
	...
	const handleTap = () => {
		if (metronomeActive) {
			...
			const sample = Math.round(
				performance.now() - metronomeBeatTimeRef.current,
			);
		...
		if (
			activeRetapRegion &&
			(correctedTimeMs < activeRetapRegion.startTimeMs ||
				correctedTimeMs > activeRetapRegion.endTimeMs)
		) {
		...
		const tooCloseToTap = taps.some(
			(tap: any) =>
				tap.passId === passState.passId &&
				Math.abs(tap.correctedTimeMs - correctedTimeMs) < 120,
		);
		if (tooCloseToTap) {
	...
	const handleUpdateReviewedAnchor = (
		anchorId: string,
		patch: any,
		triggerAutoSave = false,
	) => {
	...
	const anchorSampleCount = (anchor: any) => {
		...
		return taps.filter(
			(sample: any) =>
				sample.passId === tap.passId &&
				Math.abs(sample.correctedTimeMs - anchor.timeMs) <= 900,
		).length;
	};

	const handleAcceptGroup = (group: any) => {
		const groupAnchorIds = new Set(
			group.anchors.map((anchor: any) => anchor.id),
		);
		const nextAnchors = reviewedAnchors.map(anchor =>
			groupAnchorIds.has(anchor.id)
	...
	const handleNudgeReviewedAnchor = (anchorId: string, deltaMs: number) => {
		...
		if (!anchor) return;
		handleUpdateReviewedAnchor(
			anchorId,
			{timeMs: Math.max(0, anchor.timeMs + deltaMs)},
			true,
		);
	};
	...
	const updateTapSectionDecision = (sectionId: string, skipped: boolean) => {
		const currentCalibration = latestSongDataRef.current?.tapCalibration || {
			sectionDecisions: [],
		};
		const existingDecisions = Array.isArray(currentCalibration.sectionDecisions)
			? currentCalibration.sectionDecisions
			: [];
		const sectionDecisions = skipped
			? [
					...existingDecisions.filter(
						(decision: any) => decision.sectionId !== sectionId,
					),
					{
						sectionId,
						status: 'not_needed',
		...
		showToast(
			skipped
				? 'Section marked not needed.'
				: 'Section reopened for tap calibration.',
		);
	};
	...
	const seekToSectionWithoutAutoplay = (section: any, paddingMs = 500) => {
		setFocusedSectionId(section.id);
		throttledSeek(section.startTimeMs / 1000, true);
		zoomTimelineToRange(section.startTimeMs, section.endTimeMs, paddingMs);
		try {
			player?.pauseVideo?.();
		} catch (err) {
			console.warn(err);
	...
	const handleStartRetapRegion = (group: any) => {
		...
		const section = editorSections.find(
			sec =>
				group.startTimeMs >= sec.startTimeMs &&
				group.startTimeMs < sec.endTimeMs,
		);
		if (!section) {
	...
	const handleUpdateSectionTimes = (
		id: string,
		...
		if (field === 'endTimeMs' && secIdx === N - 1) {
			...
			if (
				numericVal <= section.startTimeMs + minDurMs ||
				numericVal > maxDurationMs
			) {
			...
			showToast(
				numericVal < maxDurationMs - minDurMs
					? 'Remaining song section created.'
					: 'Section boundary saved.',
			);
			return true;
		}
		...
		if (
			numericVal <= leftSection.startTimeMs + minDurMs ||
			numericVal >= rightSection.endTimeMs - minDurMs ||
			numericVal > maxDurationMs
		) {
			showToast('Time would make a section too short.');
			return false;
	...
	const handleAddNewSection = () => {
		...
		if (targetIdx !== -1) {
			...
			if (
				playheadMs - target.startTimeMs < 100 ||
				target.endTimeMs - playheadMs < 100
			) {
	...
	const handleDeleteSection = (id: string) => {
		...
		if (idx > 0) {
			updated[idx - 1] = {
				...updated[idx - 1],
				endTimeMs: updated[idx].endTimeMs,
			};
		} else {
		...
		if (focusedSectionId === id)
			setFocusedSectionId(updated[Math.max(0, idx - 1)]?.id ?? null);
		showToast('🗑️ Section removed.');
	};

	const handleAddEvent = (draft: DanceEventDraft) => {
		const result = addDanceEvent(
			latestSongDataRef.current?.events || [],
			draft,
			duration * 1000,
		);
		if (result.error) {
		...
		const newIndex = result.events.findIndex(
			event =>
				event.startTimeMs === draft.startTimeMs &&
				event.endTimeMs === draft.endTimeMs,
		);
	...
	const handleAddEventRangeAtPlayhead = () => {
		...
		const rangeEndMs =
			scopedSection &&
			rawStartTimeMs >= scopedSection.startTimeMs &&
			rawStartTimeMs < scopedSection.endTimeMs
				? scopedSection.endTimeMs
				: songEndMs;
		const startTimeMs = Math.max(
			rangeStartMs,
			Math.min(Math.max(rangeStartMs, rangeEndMs - 1000), rawStartTimeMs),
		);
		const defaultDurationMs = Math.min(
			3000,
			Math.max(1000, rangeEndMs - startTimeMs),
		);
		handleAddEvent({
			startTimeMs,
	...
	const handleUpdateEventTimes = (
		...
		if (field === 'startTimeMs') {
			nextStartMs = Math.max(
				0,
				Math.min(Math.round(valueMs), currentEndMs - minDurMs),
			);
		} else {
			nextEndMs = Math.min(
				maxDurationMs,
				Math.max(Math.round(valueMs), currentStartMs + minDurMs),
			);
		}
		...
		updateEventsState(events, true);
		throttledSeek(
			(field === 'startTimeMs' ? nextStartMs : nextEndMs) / 1000,
			false,
		);
	...
	const handlePublishSong = () => {
		...
		const validation = createVocabularySongMapSchema(
	...
	const seekTimelineFromClientX = (clientX: number, immediate: boolean) => {
		...
		throttledSeek(
			(visibleTimeline.startTimeMs + ratio * visibleDurationMs) / 1000,
			immediate,
		);
	};
	...
	const zoomTimelineToRange = (
		...
		setTimelineZoom({
			startTimeMs: paddedStart,
			endTimeMs: Math.max(paddedStart + 1000, paddedEnd),
		});
	};
	...
	const showSectionEventTimeline = (
		section = editorSections.find(sec => sec.id === focusedSectionId) ||
			editorSections.find(
				sec =>
					liveDisplayTime * 1000 >= sec.startTimeMs &&
					liveDisplayTime * 1000 <= sec.endTimeMs,
	...
	const zoomTimelineBy = (factor: number) => {
		...
		const nextWindow = Math.max(
			5000,
			Math.min(songEndMs, currentWindow * factor),
		);
		const center = Math.max(
			0,
			Math.min(
				songEndMs,
				liveDisplayTime * 1000 ||
					visibleTimeline.startTimeMs + currentWindow / 2,
			),
		);
		const startTimeMs = Math.max(
			0,
			Math.min(songEndMs - nextWindow, center - nextWindow / 2),
		);
	...
	const sortedEvents =
		latestSongDataRef.current?.events ||
		calibratedSongData?.events ||
		songData?.events ||
		[];
	const activeEventSection =
		activeTab === 2 && eventTimelineScope === 'section'
			? editorSections.find(section => section.id === focusedSectionId)
			: null;
	const activeEventSectionDurationMs = activeEventSection
		? activeEventSection.endTimeMs - activeEventSection.startTimeMs
		: 0;
	const activeEventSectionTimeMs = activeEventSection
		? Math.max(
				0,
				Math.min(
					activeEventSectionDurationMs,
					liveDisplayTime * 1000 - activeEventSection.startTimeMs,
				),
			)
		: 0;
	const eventDraftRangeStartMs = activeEventSection
		? activeEventSection.startTimeMs
		: 0;
	const eventDraftRangeEndMs = activeEventSection
		? activeEventSection.endTimeMs
		: Math.round(duration * 1000);
	const defaultEventDraftStartMs = Math.max(
		eventDraftRangeStartMs,
		Math.min(
			Math.max(eventDraftRangeStartMs, eventDraftRangeEndMs - 1000),
			Math.round(liveDisplayTime * 1000),
		),
	);
	const defaultEventDraftEndMs = Math.min(
		eventDraftRangeEndMs,
		defaultEventDraftStartMs + 3000,
	);
	...
	const sectionLane = isTapCockpit
		? {top: 0, height: 24}
		: timelineView === 'sections'
			? {top: 0, height: 104}
			: {top: 0, height: 36};
	const eventLane = isTapCockpit
		? {top: 27, height: 14}
		: {top: 38, height: 22};
	const rawTapLane = isTapCockpit
		? {top: 45, height: 10}
		: {top: 64, height: 16};
	const reviewedLane = isTapCockpit
		? {top: 58, height: 12}
		: {top: 83, height: 17};
			...
			>
				<div
					style={{
						...
						alignItems: 'center',
						marginBottom: isTapCockpit ? '0' : '8px',
					}}
				>
						...
						return (
							<button
								...
								onClick={() => {
									if (isLocked) {
										showToast(
											'Add at least Intro + Rest sections before event calibration.',
										);
										return;
									}
									setActiveTab(tabNum);
				...
				{!sectionStructureReady && (
					...
					>
						<span>
							Events unlock after the song has at least an Intro section plus
							one later section covering the full song.
						</span>
					</div>
				)}
						...
						>
								...
								>
									<div
										style={{
											...
											borderRadius: '50%',
											background:
												metronomeActive && metronomeBeat % 2 === 1
													? '#ffffff'
									...
									>
										<span>
											...
											<strong style={{color: '#fff'}}>
												{deviceCalibration
													? `${deviceCalibration.inputLatencyMs}ms`
													: 'none'}
											</strong>
										</span>
										<span>
											...
											<strong style={{color: '#fff'}}>
												{deviceCalibration
													? `${deviceCalibration.consistencyMs}ms`
													: 'none'}
											</strong>
										</span>
										<span>
								...
								<div style={{display: 'flex', gap: '8px'}}>
									<button
										...
										style={{
											...
											background: metronomeActive
												? '#ffffff'
												: 'rgba(255,255,255,0.05)',
											color: metronomeActive ? '#000' : '#fff',
											fontWeight: 900,
											cursor: 'pointer',
										}}
									>
							...
							>
								...
								>
									<span>
										Raw taps:{' '}
										<strong style={{color: '#fff'}}>{taps.length}</strong>
									</span>
									<span>
										Reviewed:{' '}
										<strong style={{color: '#fff'}}>
											{
												activeReviewedAnchors.filter(anchor => anchor.reviewed)
													.length
											}
										</strong>
									</span>
									...
									<span>
										Groups:{' '}
										<strong style={{color: '#fff'}}>{tapGroups.length}</strong>
									</span>
								</div>
								{activeRetapRegion && (
									<div
									...
									>
										<span>
											{formatTimelineTime(activeRetapRegion.startTimeMs)}-
											{formatTimelineTime(activeRetapRegion.endTimeMs)}
										</span>
										<button
											onClick={handleStopRetapRegion}
											style={{
												padding: '2px 7px',
												borderRadius: '6px',
								...
								>
									...
									<span
										style={{
											fontSize: '0.66rem',
											color: tapFlash ? 'rgba(0,0,0,0.6)' : '#71717a',
										}}
									>
										Click or press{' '}
										<kbd
						...
						<div style={{display: 'flex', flexDirection: 'column', gap: '8px'}}>
							...
							>
								{tapSectionCards.map((card: any) => {
									const {section} = card;
									const active =
										activeTapSectionId === section.id ||
										activeRetapRegion?.sectionId === section.id;
									const statusColor =
										card.status === 'Calibrated'
											? '#34d399'
											: card.status === 'Review'
												? '#fbbf24'
												: card.status === 'Marked Not Needed'
													? '#a1a1aa'
													: '#93c5fd';
									const labelText = section.category
										? getCategoryLabel(section.category)
										: `Section ${card.index + 1}`;
									return (
										<div
											key={`tap-section-${section.id}`}
											style={{
												display: 'flex',
												flexDirection: 'column',
												gap: '8px',
										...
										>
											...
											>
												...
												>
													...
													>
														{formatTimelineTime(section.startTimeMs)}-
														{formatTimelineTime(section.endTimeMs)}
													</span>
												</div>
												<span
													style={{
											...
											<div
												...
												{card.hiddenEvents.length > 0 && (
													<span>
														{card.hiddenEvents.length} hidden event
														{card.hiddenEvents.length === 1 ? '' : 's'}
													</span>
												)}
											</div>
											<div
											...
											>
												<button
													onClick={() => handleStartSectionTapRound(section)}
													style={{
														padding: '4px 8px',
														borderRadius: '6px',
												...
												>
													{card.markedNotNeeded || card.sectionHidden
														? 'Calibrate Anyway'
														: 'Calibrate Section'}
												</button>
												<button
													onClick={() => handleVerifySection(section)}
													style={{
														padding: '4px 8px',
														borderRadius: '6px',
														border: '1px solid rgba(244,114,182,0.35)',
														background:
															verificationGroupId === `section:${section.id}`
												...
												<button
													...
													style={{
														...
														borderRadius: '6px',
														border: '1px solid rgba(255,255,255,0.12)',
														background: card.markedNotNeeded
															? '#ffffff'
															: 'transparent',
														color: card.markedNotNeeded ? '#000' : '#d4d4d8',
														fontSize: '0.68rem',
														fontWeight: 900,
														cursor: 'pointer',
													}}
									...
									>
										...
										>
											...
											>
												...
												>
													Group {group.index + 1}:{' '}
													{(group.startTimeMs / 1000).toFixed(2)}s-
													{(group.endTimeMs / 1000).toFixed(2)}s
												</span>
												<span style={{color: '#a1a1aa', fontSize: '0.68rem'}}>
													{group.anchors.length} anchors · {group.pattern} ·
													median gap{' '}
													{(group.medianGapMs / 1000 || 0).toFixed(2)}s ·{' '}
													{group.reasons.join(', ')}
												</span>
											</div>
											<span
										...
										<div style={{display: 'flex', gap: '6px'}}>
											...
											<button
												onClick={() => handleMarkGroupUncertain(group)}
												style={{
													padding: '4px 8px',
													borderRadius: '6px',
													border: '1px solid rgba(251,191,36,0.35)',
											...
											<button
												onClick={() => handleStartRetapRegion(group)}
												style={{
													padding: '4px 8px',
													borderRadius: '6px',
													border: '1px solid rgba(96,165,250,0.35)',
										...
										{expanded &&
											group.anchors.map((anchor: any) => {
												const rawTap = taps.find(
													(tap: any) => tap.id === anchor.tapId,
												);
												...
												return (
													...
													>
														...
														>
															{(anchor.timeMs / 1000).toFixed(2)}s
														</span>
														<div
															style={{
																display: 'flex',
														...
														>
															...
															{reviewedAnchorOptions.map(option => (
																<button
																	...
																	style={{
																		padding: '3px 9px',
																		borderRadius: '999px',
																		border: `1px solid ${anchor.count === option.count ? '#ffffff' : 'rgba(255,255,255,0.12)'}`,
																		background:
																			anchor.count === option.count
																				? '#ffffff'
																				: 'transparent',
																		color:
																			anchor.count === option.count
															...
															<button
																onClick={() =>
																	handleNudgeReviewedAnchor(anchor.id, -50)
																}
																style={{
																	padding: '3px 7px',
																	borderRadius: '6px',
															...
															<button
																onClick={() =>
																	handleUpdateReviewedAnchor(
																		anchor.id,
																		{confidence: 'uncertain', reviewed: true},
																		true,
																	)
																}
																style={{
														...
														<div style={{display: 'flex', gap: '6px'}}>
															<button
																onClick={() =>
																	handleLoopReviewedAnchor(anchor.timeMs)
																}
																style={{
																	padding: '3px 7px',
																	borderRadius: '6px',
															...
															<button
																onClick={() =>
																	rawTap && handleDeleteRawTap(rawTap.id)
																}
																style={{
																	padding: '3px 7px',
																	borderRadius: '6px',
				...
				<div
					className="dev-widescreen-top-row"
					style={{
						gridTemplateColumns:
							activeTab === 3
								? 'minmax(360px, 420px) minmax(320px, 1fr)'
				...
				>
							...
							{activeTab === 3 && verificationGroup && (
								...
								>
									...
									>
										...
										>
											Verify {(verificationGroup.startTimeMs / 1000).toFixed(2)}
											s-{(verificationGroup.endTimeMs / 1000).toFixed(2)}s
										</div>
										{nextVerificationAnchor && (
											...
											>
												Next {anchorLabel(nextVerificationAnchor.count)} in{' '}
												{Math.max(
													0,
													nextVerificationAnchor.timeMs / 1000 -
														liveDisplayTime,
												).toFixed(2)}
												s
											</div>
									...
									{currentVerificationAnchor && (
										<div
											key={`${currentVerificationAnchor.id}-${Math.round(liveDisplayTime * 10)}`}
											style={{
												width: '148px',
												height: '148px',
												borderRadius: '50%',
												...
												background:
													currentVerificationAnchor.confidence === 'uncertain'
														? 'rgba(251,191,36,0.16)'
														: currentVerificationAnchor.reviewed
										...
										>
											<span
												style={{
													color: '#fff',
													fontSize:
														currentVerificationAnchor.count === 4 ||
														currentVerificationAnchor.count === 8
															? '1.45rem'
															: '3rem',
													fontWeight: 1000,
					...
					{activeTab === 3 && (
						<div
							...
							style={{
								...
								padding: '8px',
								borderRadius: '12px',
								border: `2px solid ${tapFlash ? '#ffffff' : 'rgba(255,255,255,0.08)'}`,
						...
						>
								...
								>
									...
									>
										{activeTapSectionCard
											? `${formatTimelineTime(activeTapSectionCard.section.startTimeMs)}-${formatTimelineTime(activeTapSectionCard.section.endTimeMs)}`
											: 'No section selected'}
									</span>
								</div>
								<button
									...
									style={{
										...
										borderRadius: '6px',
										border: '1px solid rgba(255,255,255,0.12)',
										background: !deviceCalibration
											? 'rgba(251,191,36,0.12)'
											: 'transparent',
										color: !deviceCalibration ? '#fbbf24' : '#d4d4d8',
										fontSize: '0.66rem',
										fontWeight: 900,
										cursor: 'pointer',
									}}
							...
							>
								...
								>
									...
									<strong
										title="Keyboard/headphone delay subtracted from your tap times. Saved in localStorage for this browser."
										style={{color: '#fff', fontSize: '0.68rem'}}
									>
										{deviceCalibration
											? `${deviceCalibration.inputLatencyMs}ms`
											: 'not set'}
							...
							{activeTapSectionCard && (
								...
								>
									...
									>
										...
										>
											{activeTapSectionCard.section.category
												? getCategoryLabel(
														activeTapSectionCard.section.category,
													)
												: `Section ${activeTapSectionCard.index + 1}`}
										</span>
										<span
											style={{
												color:
													activeTapSectionCard.status === 'Calibrated'
														? '#34d399'
														: activeTapSectionCard.status === 'Review'
															? '#fbbf24'
															: activeTapSectionCard.status ===
																  'Marked Not Needed'
																? '#a1a1aa'
																: '#93c5fd',
												fontSize: '0.65rem',
									...
									>
										...
										{activeTapSectionCard.hiddenEvents.length > 0 && (
											<span>
												{activeTapSectionCard.hiddenEvents.length} hidden break
												{activeTapSectionCard.hiddenEvents.length === 1
													? ''
													: 's'}
											</span>
										)}
									...
									<>
										...
										<button
											...
											style={{
												...
												background:
													verificationGroupId ===
													`section:${activeTapSectionCard.section.id}`
														? 'rgba(244,114,182,0.22)'
														: 'rgba(244,114,182,0.08)',
												color: '#f9a8d4',
												fontSize: '0.68rem',
										...
										<button
											...
											style={{
												...
												borderRadius: '6px',
												border: '1px solid rgba(255,255,255,0.12)',
												background: activeTapSectionCard.markedNotNeeded
													? '#ffffff'
													: 'transparent',
												color: activeTapSectionCard.markedNotNeeded
													? '#000'
													: '#d4d4d8',
												fontSize: '0.68rem',
												fontWeight: 900,
					...
					{activeTab === 2 && (
						<div
							className="glass-panel dev-panel right-workspace-column"
							style={{
								display: 'flex',
								flexDirection: 'column',
								gap: '16px',
				...
				<div
					style={{
						display: 'flex',
						flexDirection: 'column',
						gap: activeTab === 3 ? '6px' : '10px',
						background: 'rgba(255,255,255,0.015)',
						border: '1px solid rgba(255,255,255,0.05)',
						borderRadius: '16px',
						padding: activeTab === 3 ? '10px 14px' : '16px 20px',
					}}
				>
					<div
						style={{
					...
					>
						...
						<div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
							{activeTab === 2 ? (
								<>
									<button
										onClick={showWholeEventTimeline}
										style={{
											fontSize: '0.65rem',
											padding: '3px 8px',
											borderRadius: '10px',
											border: '1px solid rgba(255,255,255,0.12)',
											background:
												eventTimelineScope === 'song' ? '#fff' : 'transparent',
											color: eventTimelineScope === 'song' ? '#000' : '#71717a',
											cursor: 'pointer',
											fontWeight: 800,
										}}
									>
									...
									<button
										onClick={() => showSectionEventTimeline()}
										style={{
											fontSize: '0.65rem',
											padding: '3px 8px',
											borderRadius: '10px',
											...
											background:
												eventTimelineScope === 'section'
													...
													: 'transparent',
											color:
												eventTimelineScope === 'section' ? '#000' : '#71717a',
											cursor: 'pointer',
											fontWeight: 800,
										}}
									>
								...
								<>
									{(['sections', 'events', 'taps', 'all'] as const).map(
										view => (
											<button
												key={view}
												...
												style={{
													...
													border: '1px solid rgba(255,255,255,0.12)',
													background:
														timelineView === view ? '#fff' : 'transparent',
													color: timelineView === view ? '#000' : '#71717a',
													cursor: 'pointer',
								...
								<>
									<button
										...
										style={{
											...
											borderRadius: '10px',
											border: '1px solid rgba(255,255,255,0.12)',
											background: timelineZoom
												? 'transparent'
												: 'rgba(255,255,255,0.9)',
											color: timelineZoom ? '#a1a1aa' : '#000',
											cursor: 'pointer',
											fontWeight: 800,
										}}
									>
									...
									<button
										...
										style={{
											...
											borderRadius: '10px',
											border: '1px solid rgba(255,255,255,0.12)',
											background: followPlayhead ? '#fff' : 'transparent',
											color: followPlayhead ? '#000' : '#71717a',
											cursor: 'pointer',
											fontWeight: 800,
										}}
									>
						...
						>
							...
							>
								...
								>
									...
									>
										{`${verificationGroup.id}`.startsWith('section:')
											? 'Verifying Section'
											: `Verifying Group ${verificationGroup.index + 1}`}
									</span>
									<span
										style={{
											color: '#f9a8d4',
									...
									>
										{(verificationGroup.startTimeMs / 1000).toFixed(2)}s-
										{(verificationGroup.endTimeMs / 1000).toFixed(2)}s
									</span>
									<span
										style={{
											color:
												verificationGroup.confidence === 'high'
													? '#34d399'
													: verificationGroup.confidence === 'medium'
														? '#fbbf24'
														: '#fca5a5',
											fontSize: '0.68rem',
											fontWeight: 900,
							...
							>
								<button
									onClick={handleVerifyLooksRight}
									style={{
										padding: '5px 9px',
										borderRadius: '6px',
										border: '1px solid rgba(52,211,153,0.35)',
								...
								<button
									onClick={handleVerifyRetapAgain}
									style={{
										padding: '5px 9px',
										borderRadius: '6px',
										border: '1px solid rgba(96,165,250,0.35)',
					...
					<div
						style={{
							position: 'relative',
							padding: activeTab === 3 ? '4px 0' : '8px 0',
						}}
					>
						<div
							ref={timelineRef}
						...
						>
							...
							>
								...
								{timelineModeShowsSections &&
									editorSections
										.filter(sec =>
											timelineRangeVisible(sec.startTimeMs, sec.endTimeMs),
										)
										.map((sec, idx) => {
											const widthPct = timelineWidthPct(
												clampVisibleTime(sec.startTimeMs),
												clampVisibleTime(sec.endTimeMs),
											);
											const leftPct = timelinePct(
												clampVisibleTime(sec.startTimeMs),
											);
											const color =
								...
								{timelineModeShowsEvents &&
									sortedEvents.map((event: any, index: number) => {
										if (
											!timelineRangeVisible(event.startTimeMs, event.endTimeMs)
										)
											return null;
										const leftPct = timelinePct(
											clampVisibleTime(event.startTimeMs),
										);
										const widthPct = timelineWidthPct(
											clampVisibleTime(event.startTimeMs),
											clampVisibleTime(event.endTimeMs),
										);
								...
								{timelineModeShowsRawTaps &&
									activeTaps
										.filter(
											(tap: any) =>
												tap.correctedTimeMs >= visibleTimeline.startTimeMs &&
												tap.correctedTimeMs <= visibleTimeline.endTimeMs,
										)
										.map((tap: any) => (
											<div
												key={tap.id}
												title={`Raw anchor ${(tap.correctedTimeMs / 1000).toFixed(2)}s`}
												style={{
													position: 'absolute',
													top: `${rawTapLane.top}px`,
													height: `${rawTapLane.height}px`,
													left: `${timelinePct(tap.correctedTimeMs)}%`,
													width: '2px',
													background: '#a1a1aa',
													opacity: 0.72,
													zIndex: 8,
								...
								{activeTab === 3 &&
									...
									verificationAnchors
										...
										.map((anchor: any) => {
											...
											return (
												<div
													key={`verify-${anchor.id}`}
													title={`Verify ${anchorLabel(anchor.count)} ${(anchor.timeMs / 1000).toFixed(2)}s`}
													style={{
														position: 'absolute',
							...
							{activeTab === 1 &&
								editorSections.map((sec, idx) => {
									...
									if (
										sec.endTimeMs < visibleTimeline.startTimeMs ||
										sec.endTimeMs > visibleTimeline.endTimeMs
									)
										return null;
									...
									return (
										<div
											...
											onMouseDown={e => {
												...
												e.preventDefault();
												const handleMouseMove = (moveEvt: MouseEvent) => {
													handleUpdateSectionTimes(
														sec.id,
														'endTimeMs',
														timeFromTimelineClientX(moveEvt.clientX),
													);
												};
												const handleMouseUp = () => {
							...
							{activeTab === 2 &&
								sortedEvents.flatMap((event: any, index: number) => {
									if (!timelineRangeVisible(event.startTimeMs, event.endTimeMs))
										return [];
									...
									return [
										<div
											...
											style={{
												position: 'absolute',
												left: `${startPct}%`,
												top: `${eventLane.top - 6}px`,
												width: '12px',
												height: `${eventLane.height + 12}px`,
												transform: 'translateX(-50%)',
												cursor: 'col-resize',
												zIndex: 22,
												display: 'flex',
					...
					{activeTab === 1 && editorSections.length > 0 && (
						...
						>
							{editorSections.map((sec, idx) => {
								...
								const isActive = sec.id === focusedSectionId;
								const labelText = sec.category
									? getCategoryLabel(sec.category)
									: `Section ${idx + 1}`;
								return (
									<button
										key={sec.id}
										onClick={() => {
											...
											if (!isActive) {
												seekToSectionWithoutAutoplay(sec);
											}
										}}
										style={{
											fontSize: '0.68rem',
							...
							{editorSections.map((sec, idx) => {
								...
								return (
									<button
										...
										onClick={() => {
											seekToSectionWithoutAutoplay(sec, 1000);
											showSectionEventTimeline(sec);
										}}
										style={{
											fontSize: '0.68rem',
								...
								return (
									...
									>
										...
										{!card.sectionHidden && card.hiddenEvents.length > 0 && (
											...
											>
												{card.hiddenEvents.length} break
												{card.hiddenEvents.length === 1 ? '' : 's'}
											</span>
										)}
									</button>
					...
					{activeTab === 3 && (
						...
						>
							<button
								onClick={() => setShowTapAdvancedReview(current => !current)}
								style={{
									alignSelf: 'flex-start',
									padding: '5px 9px',
									borderRadius: '6px',
									border: '1px solid rgba(255,255,255,0.12)',
									background: showTapAdvancedReview
										? '#ffffff'
										: 'rgba(255,255,255,0.04)',
									color: showTapAdvancedReview ? '#000' : '#d4d4d8',
									fontSize: '0.68rem',
									fontWeight: 900,
									cursor: 'pointer',
								}}
											...
											>
												...
												>
													...
													>
														...
														>
															Group {group.index + 1}:{' '}
															{formatTimelineTime(group.startTimeMs)}-
															{formatTimelineTime(group.endTimeMs)}
														</span>
														<span
															style={{color: '#a1a1aa', fontSize: '0.68rem'}}
														>
															{group.anchors.length} anchors · {group.pattern} ·{' '}
															{group.reasons.join(', ')}
														</span>
													...
													group.anchors.map((anchor: any) => {
														...
														return (
															...
															>
																...
																>
																	{formatTimelineTime(anchor.timeMs)}
																</span>
																<div
																	style={{
																		display: 'flex',
					...
					>
						...
						>
							...
							>
								...
								>
									Measures the delay between hearing the metronome and your
									keyboard tap. This is saved in this browser only and is
									subtracted from future tap times.
								</span>
							</div>
							<button
								onClick={closeOffsetCalibrationModal}
								style={{
									padding: '5px 9px',
									borderRadius: '7px',
									border: '1px solid rgba(255,255,255,0.12)',
						...
						>
							...
							>
								<span
									style={{color: '#fff', fontSize: '0.82rem', fontWeight: 900}}
								>
									{metronomeActive
										? 'Tap Sample when you hear each click'
										: 'Start the metronome, then tap samples'}
								</span>
								<span style={{color: '#a1a1aa', fontSize: '0.72rem'}}>
									Samples: {metronomeSamples.length}. Use the same keyboard and
									headphones you will use for song tapping.
								</span>
							</div>
						</div>
						...
						<div
							style={{
								display: 'grid',
								gridTemplateColumns: metronomeActive ? '1fr 1fr' : '1fr',
								gap: '8px',
							}}
						>
							<button
								...
								style={{
									...
									background: metronomeActive
										? '#ffffff'
										: 'rgba(96,165,250,0.12)',
									color: metronomeActive ? '#000' : '#93c5fd',
									fontWeight: 900,
									cursor: 'pointer',
								}}
							>
							...
							{metronomeActive && (
								<button
									...
									style={{
										...
										borderRadius: '8px',
										border: '1px solid rgba(255,255,255,0.16)',
										background: tapFlash ? '#ffffff' : 'rgba(255,255,255,0.05)',
										color: tapFlash ? '#000' : '#fff',
										fontWeight: 900,
										cursor: 'pointer',
									}}
						...
						>
							<span
								style={{color: '#71717a', fontSize: '0.68rem', fontWeight: 800}}
							>
								Storage: localStorage key {DEVICE_CALIBRATION_KEY}
							</span>
							{deviceCalibration && (
								<button
									onClick={clearDeviceCalibration}
									style={{
										padding: '7px 10px',
										borderRadius: '7px',
										border: '1px solid rgba(248,113,113,0.35)',
